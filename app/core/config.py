...
class Settings(BaseSettings):
    ...
    welcome_message: str = Field(default=os.getenv("WELCOME_MESSAGE", "Добро пожаловать на сервер, {member.mention}!"), alias="WELCOME_MESSAGE")
...
def get_settings() -> Settings:
    ...
    try:
        ...
        raise RuntimeError(
            "Config validation error: DISCORD_TOKEN/OWNER_ID required (зайте через Fly secrets или .env)"
        ) from e
